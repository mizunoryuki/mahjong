...
最初のリリース目標は、自動検証・外部照合済み15〜20問を使った診断プロトタイプ（Milestone 3a）とする。Milestone 3bの利用者テストに合格するまで、64問制作と公開βへ進まない。
...
- M3a前にhan・fu・payoutの類題ペアを含む15〜20問へ完全内訳・自動検証・外部照合証跡を揃える。
...
- [x] 開発用問題バンクを15問まで増やす。
- [x] Rule ownerを実装責任者が兼任できる運用へ変更し、採用範囲と対象外を文書化する。
- [x] 15問にMリーグ公式資料と異なる外部資料2件の照合証跡を記録する。
- [x] 全15問へ完全な手牌分解とScoringBasisを記録する。
- [x] 全自動検査が揃った15問を`published`にする。
