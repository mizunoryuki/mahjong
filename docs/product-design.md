...
> 監修方針は[ADR-004](adr-004-automated-content-verification.md)により更新された。本文中の「人間の独立監修・二重監修を必須」とする記述は、自動検証・Mリーグ公式資料・独立した外部資料2系統の照合へ読み替える。
...
`resolveFu`は標準形の部品合計と10符切り上げを検証し、七対子25符、平和ツモ20符、喰い平和形30符を判別する。Mリーグ公式戦ルールに従い、3飜60符と4飜30符は切り上げ満貫（子8,000点・親12,000点）とする。連風牌の雀頭は2符とする。自然役満は単一の`yakumanId`を持つ`kind="yakuman"`、13飜以上の通常手は入力拒否するため両者を混同しない。MVPは複数役満を型で表現しない。`basis.closed`は`melds`が暗槓だけかどうかから導出した値と一致しなければならない。
...
最低golden fixtureは、通常点の1〜4飜20/25/30/40/60/70符、3飜60符・4飜30符の切り上げ満貫、満貫・跳満・倍満・三倍満、単一自然役満、七対子、平和ツモ、親子のロン・ツモ全分岐を含む。食い下がり対象の全役は門前/副露を対にし、三色同順2/1飜、一気通貫2/1飜、混一色3/2飜、清一色6/5飜などを固定fixtureにする。代表値は次のとおり。

| ケース | 期待支払い |
|---|---|
| 子1飜30符ロン / 3飜40符ロン | 1,000点 / 5,200点 |
| 子3飜60符ロン / 4飜30符ロン | 8,000点 |
